...
| `blog_scroll_50` | scroll depth ≥ 50% on a post | `slug` |
| `blog_scroll_90` | scroll depth ≥ 90% on a post | `slug` |
...
- What's the conversion ratio (`blog_cta_click` / `blog_post_viewed`)? Anything above 3% is excellent. 1–3% is healthy. Under 1% means the close isn't doing its job.
